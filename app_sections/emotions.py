...
def set_emotional_section(df: pd.DataFrame) -> None:
    ...
    df_with_mistakes = df[(df["mistake"] != "No data") & (df["mistake"] != "No mistake")].copy()
    metric_col = st.columns(1)
    metric_col[0].metric("Total Mistakes:", len(df_with_mistakes))

    chart_cols = st.columns(3)

    with chart_cols[0]:
        with st.container(border=True):
            if df_with_mistakes.empty:
                st.warning("No data")

            st.subheader("Mistakes Count")
            mistake_counts = df_with_mistakes["mistake"].value_counts().reset_index()
    ...
    with chart_cols[1]:
        with st.container(border=True):
            ...
            session_data = df_with_mistakes.groupby(["trade_session"])["mistake"].count().reset_index()
    ...
    with chart_cols[2]:
        with st.container(border=True):
            ...
            grouped_df = df_with_mistakes.groupby(["mistake"])["profit"].sum().reset_index()
